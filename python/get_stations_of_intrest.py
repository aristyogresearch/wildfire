...
logging.basicConfig(
    filename=config.get_stations_of_intrest_log, level=logging.DEBUG)
...
def main():
    ...
    source_sensors = open(config.target_sensors_list, 'r')
    ...
    station_sensors = open(config.station_sensors_list, 'r')
    ...
    output = open(config.target_stations_list, 'w')
    output.write(config.target_stations_header)
